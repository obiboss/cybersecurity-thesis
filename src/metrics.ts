...
/**
 * risk_pressure:
 *  - Increases during threats
 *  - Decays automatically over time
 */
...
/* -------------------------------------------------------------------------- */
/*                          RISK PRESSURE DECAY MODEL                          */
/* -------------------------------------------------------------------------- */

/**
 * Decays risk pressure gradually (called periodically)
 * Example: -5 every minute
 */
export function decayRiskPressure(step = 5): SystemMetrics {
  const m = getMetrics();
  if (m.risk_pressure === 0) return m;

  return setMetrics({
    risk_pressure: Math.max(0, m.risk_pressure - step),
  });
}
