...
import { db } from "../db.js";
import { requireAuth, requireRole } from "./auth.js";
import { recalcSecurityScore } from "../metrics.js";
import { emitEvent } from "../websocket.js";
...
incidentsRouter.get(
  ...
  (req: Request, res: Response) => {
    const rows = db
      .prepare("SELECT * FROM incidents ORDER BY detected_at DESC LIMIT 200")
      .all();
    res.json({ items: rows });
  }
);
...
incidentsRouter.patch(
  ...
  (req: Request, res: Response) => {
    ...
    db.prepare(
      ...
      Number(req.userId), // handled via type augmentation (see below)
      "update_incident_status",
      `incident_id=${id}; status=${status}`
    );
